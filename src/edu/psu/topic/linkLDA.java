...
public class linkLDA extends Model {
	...
	public static void main(String[] args) {
		...
		linkLDA linkLDA = new linkLDA();
		System.out.println("Reading Data.....");
		Corpus corpus = linkLDA.readData(input);
		...
		for (int length = 0; length < windowLength; length++) {
			...
			if (false) {
				...
				for (int i = 0; i < corpus.docs.size(); i++) {
					...
					int[] citations = doc.citationSet.keys();
					for (int j = 0; j < citations.length; j++) {
						double[] tmpKL = new double[3];// linkLDA.calculateJSDiv(corpus,
														// i, citations[j],
														// shortc);
